# -*- coding: utf-8 -*-
"""Proyecto_Grupo_8_gbcd (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1064etAXcV6FRDDhQt_bxovtkmQ191s_p

# Predicción de Suscripción a Depósito a Plazo fijo — Bank Marketing
"""
...
Las campañas de telemarketing continúan siendo una herramienta ampliamente utilizada por las instituciones financieras para promocionar productos y servicios. Sin embargo, realizar contactos masivos implica altos costos operativos y una baja tasa de conversión cuando las campañas no están dirigidas al público adecuado. En este contexto, resulta fundamental identificar qué clientes tienen mayor probabilidad de suscribirse a un depósito a plazo fijo, permitiendo optimizar recursos, mejorar la eficiencia comercial y reducir contactos innecesarios.

Un banco portugués ejecutó campañas de marketing telefónico entre mayo de 2008 y noviembre de 2010 para promover sus depósitos a plazo. El histórico cuenta con 45,211 contactos con sus perfiles de clientes y resultados (si suscribieron o no el producto).

## Fuente

Los datos provienen del **UCI Machine Learning Repository** y fueron creados por **Paulo Cortez (Univ. Minho)** y **Sérgio Moro (ISCTE-IUL)** en 2012. El dataset y su análisis original están descritos en:
...
Identificar **qué factores influyen más en la decisión del cliente de suscribir un depósito a plazo fijo**, y construir un **modelo de Machine Learning** que prediga si un cliente suscribirá o no el producto (`y` = `yes`/`no`).

A lo largo del proyecto se realizará análisis exploratorio de datos (EDA), evaluación de calidad del dato, transformación de variables y aplicación de modelos de clasificación para obtener información que contribuya a mejorar la toma de decisiones comerciales.

## Diccionario de datos

El dataset tiene **45,211 instancias**, **16 atributos de entrada + 1 variable objetivo**, y **no contiene valores faltantes** (según la documentación oficial). Cada fila representa un contacto con un cliente.
...
* `marital`: Estado civil (categórica: married, divorced, single). **Nota**: "divorced" incluye divorciados y viudos.
...
* `housing`: ¿Tiene crédito hipotecario? (binaria: yes/no)
* `loan`: ¿Tiene crédito personal? (binaria: yes/no)
...
* `day`: Último día de contacto del mes (numérica)
* `month`: Último mes de contacto del año (categórica: jan, feb, mar, ..., nov, dec)
* `duration`: Duración del último contacto en segundos (numérica).

**Otros atributos:**
* `campaign`: Número de contactos realizados durante esta campaña al cliente (numérica, incluye el último contacto)
* `pdays`: Días que pasaron desde el último contacto al cliente en una campaña previa (numérica, -1 significa que no fue contactado antes)
* `previous`: Número de contactos realizados antes de esta campaña al cliente (numérica)
* `poutcome`: Resultado de la campaña de marketing previa (categórica: unknown, other, failure, success)

**Variable objetivo (target):**
* `y`: ¿El cliente suscribió un depósito a plazo? (binaria: yes/no)

## Enfoque para resolver el problema

1. Importar las librerías necesarias
2. Leer el dataset y obtener una visión general
3. Análisis exploratorio de datos (EDA) — a. Univariado  b. Bivariado
4. Preprocesamiento de datos
5. Definir la métrica de desempeño y construir modelos de ML
